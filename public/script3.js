...
loginFormElement.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                window.location.href = '/dashboard-teacher'; // Redirect to the dashboard after 2 seconds
            }, 2000);
        } else {
...
signupFormElement.addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch('/signup-teacher', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                window.location.href = '/login-teacher'; // Redirect to the login page after 2 seconds
            }, 2000);
        } else {
